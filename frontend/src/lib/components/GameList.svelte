<script lang="ts">
	...
	interface Props {
		...
		onView?: (game: GameSummary) => void;
		refreshTrigger?: number;
	}

	let { onEdit, onDelete, onView, refreshTrigger = 0 }: Props = $props();
...
<div class="w-full">
	...
	<div class="mb-6 flex items-center justify-between">
		<div>
			<h2 class="text-2xl font-bold text-gray-900">Game Library</h2>
		...
		<div class="mb-8 grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
			{#each games as game (game.id)}
				<Card class="p-6 transition-shadow hover:shadow-lg">
					<!-- Game Header -->
					<div class="mb-4">
						<h3 class="mb-1 text-lg font-semibold text-gray-900">{game.name}</h3>
						{#if game.publisher}
